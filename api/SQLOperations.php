...
// =========================================================================================================
        ...
        if (strlen(trim($email)) != 0 && strlen(trim($pass1)) != 0 && strlen(trim($pass2)) != 0 && strlen(trim($role)) != 0 && strlen(trim($name)) != 0 && strlen(trim($tel)) != 0) {
            ...
            if (Utilities::checkValidEmail($email)) {
                ...
                if ($pass1 == $pass2) {
                    ...
                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_EMAIL . "` = '$email' LIMIT 1"))) {
                    ...
                    if ($result == 0) {
                        ...
                        if (in_array($role, Constants::USER_TYPES)) {
                            ...
                            if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_EMAIL . "` = '$email',  `" . Constants::USERS_FLD_PASS . "` = '$hashedPass',  `" . Constants::USERS_FLD_NAME . "` = '$name',  `" . Constants::USERS_FLD_TEL . "` = '$tel',  `" . Constants::USERS_FLD_USER_TYPE . "` = '$role'"))) {
                                return $this->returnError(Constants::SIGNUP_OPERATION_FAILED, "Please try again later!", 0, 0, 0);
                            }
                            // 3.1. Check the role
                            ...
                            $_id = $this->db_manager->getLastInsertID();
                            switch ($role) {
                                ...
                                case Constants::USER_SELLER:
                                    ...
                                    $bankaccount = Utilities::makeInputSafe($extraData[Constants::SELLERS_FLD_BANK_ACCOUNT]);
                                    if (strlen(trim($address)) != 0 && strlen(trim($bankaccount)) != 0) {
                                        $result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_SELLERS . "` SET `" . Constants::SELLERS_FLD_ADDRESS . "` = '$address', `" . Constants::SELLERS_FLD_BANK_ACCOUNT . "` = '$bankaccount', `" . Constants::SELLERS_FLD_USER_ID . "` = '$_id'");
                                        if ($result > 0) {
                                            return $this->login($email, $pass1);
                                        } else {
                                            return $this->returnError(Constants::SIGNUP_OPERATION_FAILED, "Please try again later!", $_id, Constants::USERS_FLD_ID, Constants::TBL_USERS);
                                ...
                                case Constants::USER_ADMIN:
                                    $result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_ADMINS . "` SET `" . Constants::ADMINS_FLD_USER_ID . "` = '$_id'");
                                    if ($result > 0) {
                                        return $this->login($email, $pass1);
                                    } else {
                                        return $this->returnError(Constants::SIGNUP_OPERATION_FAILED, "Please try again later!", $_id, Constants::USERS_FLD_ID, Constants::TBL_USERS);
                                ...
                                case Constants::USER_ACCOUNTANT:
                                    $result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_ACCOUNTANTS . "` SET `" . Constants::ADMINS_FLD_USER_ID . "` = '$_id'");
                                    if ($result > 0) {
                                        return $this->login($email, $pass1);
                                    } else {
                                        return $this->returnError(Constants::SIGNUP_OPERATION_FAILED, "Please try again later!", $_id, Constants::USERS_FLD_ID, Constants::TBL_USERS);
                                ...
                                case Constants::USER_DELIVERMAN:
                                    $result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_DELIVERYMEN . "` SET `" . Constants::ADMINS_FLD_USER_ID . "` = '$_id'");
                                    if ($result > 0) {
                                        return $this->login($email, $pass1);
                                    } else {
                                        return $this->returnError(Constants::SIGNUP_OPERATION_FAILED, "Please try again later!", $_id, Constants::USERS_FLD_ID, Constants::TBL_USERS);
    ...
    public function login($email, $pass) {
        ...
        if (strlen(trim($email)) != 0 && strlen(trim($pass)) != 0) {
            ...
            if (Utilities::checkValidEmail($email)) {
                ...
                if (!$result = $this->db_manager->queryWithResult("SELECT * FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_EMAIL . "` = '$email' LIMIT 1")) {
    ...
    function generateUserModel($row) {
        ...
        switch ($userType) {
            case Constants::USER_BUYER:
                // Select more data
                if (!$resultDetails = $this->db_manager->queryWithResult("SELECT * FROM `" . Constants::TBL_BUYERS . "` WHERE `" . Constants::BUYERS_FLD_USER_ID . "` = $_id LIMIT 1")) {
                    return $this->returnError(Constants::LOGIN_OPERATION_FAILED, "Please try again later!", 0, 0, 0);
                }
            ...
            case Constants::USER_SELLER:
                // Select more data
                if (!$resultDetails = $this->db_manager->queryWithResult("SELECT * FROM `" . Constants::TBL_SELLERS . "` WHERE `" . Constants::SELLERS_FLD_USER_ID . "` = $_id LIMIT 1")) {
                    return $this->returnError(Constants::LOGIN_OPERATION_FAILED, "Please try again later!", 0, 0, 0);
                }
                $rowDetails = $resultDetails->fetch_assoc();

                $userModel = new Seller($_id, $row[Constants::USERS_FLD_NAME], $row[Constants::USERS_FLD_EMAIL], $row[Constants::USERS_FLD_TEL], $userType, $row[Constants::USERS_FLD_STATUS], $rowDetails[Constants::SELLERS_FLD_ADDRESS], $rowDetails[Constants::SELLERS_FLD_BANK_ACCOUNT]);
                break;
    ...
    function refreshUserHash($row, $pass) {
        ...
        $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_PASS . "` = '$newHash' WHERE `" . Constants::USERS_FLD_ID . "` = '$_id' LIMIT 1");
    }
    ...
    function editAccount($userID, $userType, $userNewData) {
        ...
        if ($userType == Constants::USER_SELLER) {
            ...
            $bankaccount = Utilities::makeInputSafe($userNewData[Constants::SELLERS_FLD_BANK_ACCOUNT]);
        ...
        if (strlen($name) > 0 && strlen($tel) > 0 && strlen($pass1) > 0) {
            ...
            if ($result->num_rows == 1) {
                ...
                if (Utilities::checkPassword($pass1, $password) == true) {
                    ...
                    if (-1 == ($result = $this->db_manager->queryWithNoResult($query))) {
                        return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
                    }
                    if ($userType == Constants::USER_SELLER) {
                        $query = "UPDATE `" . Constants::TBL_SELLERS . "` SET `" . Constants::SELLERS_FLD_ADDRESS . "` = '$address', `" . Constants::SELLERS_FLD_BANK_ACCOUNT . "` = '$bankaccount' WHERE `" . Constants::SELLERS_FLD_USER_ID . "` = $userID LIMIT 1";
                        if (-1 == ($result = $this->db_manager->queryWithNoResult($query))) {
                            return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
                        }
                    }
    ...
    function checkIfActiveUser($userID) {
        ...
        if (!$result = $this->db_manager->queryWithResult("SELECT `" . Constants::USERS_FLD_STATUS . "` FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_ID . "` = '$userID' LIMIT 1")) {
            return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
        }
    ...
    function editEmpAccount($data) {
        ...
        if (strlen($email) > 0 && strlen($userID) > 0) {
            if (Utilities::checkValidEmail($email)) {
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_EMAIL . "` = '$email' AND `" . Constants::USERS_FLD_ID . "` != '$userID'  LIMIT 1"))) {
                    return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
                }
                if ($result == 0) {
                    if (strlen($pass1) > 0) {
                        $hashPass = Utilities::hashPassword($pass1);
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_EMAIL . "` = '$email', `" . Constants::USERS_FLD_PASS . "` = '$hashPass' WHERE `" . Constants::USERS_FLD_ID . "` = '$userID'"))) {
                            return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
                        }
                    } else {
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_EMAIL . "` = '$email' WHERE `" . Constants::USERS_FLD_ID . "` = '$userID'"))) {
                            return $this->returnError(Constants::USER_EDIT_ACCOUNT_FAILED, "Please try again later!", 0, 0, 0);
                        }
                    }
    ...
    function getUsersUsingType($userType) {
        ...
        $allUsers = array();
        $result = $this->db_manager->queryWithResult($query);
        if (!$result) {
    ...
    function changeUserStatus($userID, $newStatus) {
        ...
        if ($newStatus == Constants::USER_ACTIVE || $newStatus == Constants::USER_BANNED) {
            $result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_STATUS . "` = '$newStatus' WHERE `" . Constants::USERS_FLD_ID . "` = '$userID' LIMIT 1");
            if (-1 == $result) {
    ...
    function deleteUser($userID) {
        ...
        if (-1 == ($result = $this->db_manager->queryWithResult("SELECT " . Constants::USERS_FLD_USER_TYPE . " FROM " . Constants::TBL_USERS . " WHERE " . Constants::USERS_FLD_ID . " = '$userID' LIMIT 1"))) {
            return $this->returnError(Constants::USER_DELETE_FAILED, "Please try again later!");
        } else {
            $row = $result->fetch_assoc();
            if ($row[Constants::USERS_FLD_USER_TYPE] == Constants::USER_DELIVERMAN) {
                if (-1 == ($result = $this->db_manager->queryWithNoResult("CALL deleteDeliveryMan($userID);"))) {
                ...
                if (-1 == ($result = $this->db_manager->queryWithNoResult("DELETE FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_ID . "` = '$userID' LIMIT 1"))) {
                    return $this->returnError(Constants::USER_DELETE_FAILED, "Please try again later!", 0, 0, 0);
                }
                if ($result == 1) {
                    $theResponse = new Response(Constants::USER_DELETE_SUCCESSFUL, "", "");
        ...
        if (strlen($email) > 0 && strlen($pass1) > 0 && strlen($empType) > 0) {
            if (Utilities::checkValidEmail($email)) {
                if (in_array($empType, Constants::USER_TYPES)) {
                    ...
                    if ($result == 0) {
                        $hashPass = Utilities::hashPassword($pass1);
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_USERS . "` SET `" . Constants::USERS_FLD_EMAIL . "` = '$email', `" . Constants::USERS_FLD_PASS . "` = '$hashPass', `" . Constants::USERS_FLD_USER_TYPE . "` = '$empType', `" . Constants::USERS_FLD_NAME . "` = 'Emp',  `" . Constants::USERS_FLD_TEL . "` = '0'"))) {
                            return $this->returnError(Constants::USER_INSERT_FAILED, "Please try again later!", 0, 0, 0);
                        }
                        $uID = $this->db_manager->getLastInsertID();
                        if ($empType == Constants::USER_ACCOUNTANT && -1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_ACCOUNTANTS . "` SET `" . Constants::ACCOUNTANTS_FLD_USER_ID . "` = '" . $uID . "'"))) {
                            return $this->returnError(Constants::USER_INSERT_FAILED, "Please try again later!", 0, 0, 0);
                        }
                        if ($empType == Constants::USER_DELIVERMAN && -1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_DELIVERYMEN . "` SET `" . Constants::DELIVERYMEN_FLD_USER_ID . "` = '" . $uID . "'"))) {
                            return $this->returnError(Constants::USER_INSERT_FAILED, "Please try again later!", 0, 0, 0);
                        }
    ...
    function getCartProducts($userID) {
        ...
        $query = "SELECT p.*, ct." . Constants::CART_ITEMS_QUANTITY . " FROM `products_view` p JOIN " . Constants::TBL_CART_ITEMS . " ct ON ct." . Constants::CART_ITEMS_PRODUCT_ID . " = p." . Constants::PRODUCTS_FLD_ID . " WHERE ct." . Constants::CART_ITEMS_USER_ID . " = '$userID' ORDER BY ct." . Constants::CART_ITEMS_ID . " DESC, p." . Constants::PRODUCTS_FLD_ID;
        if (-1 == ($result = $this->db_manager->queryWithResult($query))) {
    ...
    function addProductToCart($productId, $userID) {
        ...
        if (strlen($productId) > 0 && strlen($userID) > 0) {
            if (!$result = $this->db_manager->queryWithResult("SELECT `" . Constants::PRODUCTS_FLD_PRICE . "` FROM `" . Constants::TBL_PRODUCTS . "` WHERE `" . Constants::PRODUCTS_FLD_ID . "` = '$productId' AND `" . Constants::PRODUCTS_FLD_AVA_STATUS . "` = '" . Constants::PRODUCT_AVAILABLE . "' AND `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` > 0 ")) {
            ...
            if ($result->num_rows > 0) {
                ...
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_USERS . "` WHERE `" . Constants::USERS_FLD_ID . "` = '$userID' AND `" . Constants::USERS_FLD_STATUS . "` = '" . Constants::USER_BANNED . "'"))) {
                    return $this->returnError(Constants::CART_ADD_ITEM_FAILED, "Please try again later!", 0, 0, 0);
                }
                if ($result == 0) {
                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CART_ITEMS . "` WHERE `" . Constants::CART_ITEMS_USER_ID . "` = '$userID' AND `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId'"))) {
                        return $this->returnError(Constants::CART_ADD_ITEM_FAILED, "Please try again later!", 0, 0, 0);
                    }
                    if ($result == 0) {
                        // product isn't in cart 
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_CART_ITEMS . "` SET `" . Constants::CART_ITEMS_USER_ID . "` = '$userID', `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId', `" . Constants::CART_ITEMS_PRODUCT_TOTAL_PRICE . "` = '$productPrice'"))) {
                            return $this->returnError(Constants::CART_ADD_ITEM_FAILED, "Please try again later!", 0, 0, 0);
                        }
                    } else {
                        // product already exists
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_CART_ITEMS . "` SET `" . Constants::CART_ITEMS_QUANTITY . "` = " . Constants::CART_ITEMS_QUANTITY . " + 1 , `" . Constants::CART_ITEMS_PRODUCT_TOTAL_PRICE . "` = `" . Constants::CART_ITEMS_PRODUCT_TOTAL_PRICE . "` + '$productPrice'  WHERE `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId' AND  `" . Constants::CART_ITEMS_USER_ID . "` = '$userID' AND `" . Constants::CART_ITEMS_QUANTITY . "` <= 9 LIMIT 1"))) {
                            return $this->returnError(Constants::CART_ADD_ITEM_FAILED, "Please try again later!", 0, 0, 0);
                        }
                        if ($result == 0) {
                    ...
                    $cartItemID = $this->db_manager->getLastInsertID();
                    // reduce product qunatity 
                    if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCTS . "` SET `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` = " . Constants::PRODUCTS_FLD_AVA_QUANTITY . " - 1,  `" . Constants::PRODUCTS_FLD_SOLDITEMS . "` = " . Constants::PRODUCTS_FLD_SOLDITEMS . " + 1,  `" . Constants::PRODUCTS_FLD_EARNINGS . "` = " . Constants::PRODUCTS_FLD_EARNINGS . " + " . Constants::PRODUCTS_FLD_PRICE . "  WHERE`" . Constants::PRODUCTS_FLD_ID . "` = '$productId' AND `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` > 0 LIMIT 1"))) {
                        return $this->returnError(Constants::CART_ADD_ITEM_FAILED, "Please try again later!", 0, 0, 0);
                    }
                    if ($result == 1) {
    ...
    function removeProductFromCart($productID, $userID) {
        ...
        if (strlen($productId) > 0 && strlen($userID) > 0) {
            ...
            if (!$result = $this->db_manager->queryWithResult("SELECT `" . Constants::CART_ITEMS_QUANTITY . "` FROM `" . Constants::TBL_CART_ITEMS . "` WHERE `" . Constants::CART_ITEMS_USER_ID . "` = '$userID' AND `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId' LIMIT 1")) {
            ...
            if (-1 == ($result = $this->db_manager->queryWithNoResult("DELETE FROM `" . Constants::TBL_CART_ITEMS . "` WHERE `" . Constants::CART_ITEMS_USER_ID . "` = '$userID' AND `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId' LIMIT 1"))) {
                return $this->returnError(Constants::CART_DELETE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
            if ($result == 0) {
                return $this->returnError(Constants::CART_DELETE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
            if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCTS . "` SET `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` = " . Constants::PRODUCTS_FLD_AVA_QUANTITY . " + $quantity WHERE`" . Constants::PRODUCTS_FLD_ID . "` = '$productId' LIMIT 1"))) {
                return $this->returnError(Constants::CART_DELETE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
    ...
    function decreaseProductFromCart($productID, $userID) {
        ...
        if (strlen($productId) > 0 && strlen($userID) > 0) {
            ...
            if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_CART_ITEMS . "` SET `" . Constants::CART_ITEMS_QUANTITY . "` = " . Constants::CART_ITEMS_QUANTITY . " - 1 WHERE `" . Constants::CART_ITEMS_PRODUCT_ID . "` = '$productId' AND `" . Constants::CART_ITEMS_USER_ID . "` = '$userID' AND `" . Constants::CART_ITEMS_QUANTITY . "` > 1 LIMIT 1"))) {
                return $this->returnError(Constants::CART_DECREASE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
            if ($result == 0) {
                return $this->returnError(Constants::CART_DECREASE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
            if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCTS . "` SET `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` = " . Constants::PRODUCTS_FLD_AVA_QUANTITY . " + 1 WHERE`" . Constants::PRODUCTS_FLD_ID . "` = '$productId' LIMIT 1"))) {
                return $this->returnError(Constants::CART_DECREASE_ITEM_FAILED, "Please try again later!", 0, 0, 0);
            }
    ...
    function returnError($code, $msg, $idToDelete = 0, $column = 0, $tbl = 0) {
        ...
        if ($idToDelete > 0) {
            $result = $this->db_manager->queryWithNoResult("DELETE FROM `" . $tbl . "` WHERE `" . $column . "` = '$idToDelete'");
        }
    ...
    public function getAllOrders($selectionCols, $appliedFilters, $userID = "") {
        ...
        if ($userID != "") {
            $result = $this->db_manager->queryWithResult('SELECT ' . $selectionCols . ' FROM ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::ORDERS_STATUS_ID . ' != ' . Constants::ORDER_DELETED . ' AND ' . Constants::ORDERS_BUYER_ID . ' = ' . $userID . ' ORDER BY ' . Constants::ORDERS_STATUS_ID . ' ASC');
        } else {
            if (strlen($theWhereQuery) > 0) {
                $result = $this->db_manager->queryWithResult('SELECT ' . $selectionCols . ' FROM ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::ORDERS_STATUS_ID . ' != ' . Constants::ORDER_DELETED . ' AND ' . $theWhereQuery . ' ORDER BY ' . Constants::ORDERS_STATUS_ID . ' ASC');
            } else {
                $result = $this->db_manager->queryWithResult('SELECT ' . $selectionCols . ' FROM ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::ORDERS_STATUS_ID . ' != ' . Constants::ORDER_DELETED . ' ORDER BY ' . Constants::ORDERS_STATUS_ID . ' ASC');
            }
        }
        $ret = array();
        while ($row = $result->fetch_assoc()) {
    ...
    public function getOrder($id, $selectionCols) {
        ...
        if ($selectionCols == "") { //Select all columns
            $result = $this->db_manager->queryWithResult('SELECT * FROM ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::ORDERS_ID . ' = ' . $id . ' LIMIT 1');
            ...
            $result = $this->db_manager->queryWithResult('SELECT ' . $selectionCols . ' FROM ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::ORDERS_ID . ' = ' . $id . ' LIMIT 1');
            $theString = explode(",", $selectionCols);
        }
    ...
    public function addOrder($buyerId) {
        ...
        if ($this->db_manager->db_link->multi_query("CALL addOrder($buyerId);")) {
            do {
                /* store first result set */
                if ($result = $this->db_manager->db_link->store_result()) {
                    while ($row = $result->fetch_assoc()) {
                        $orderID = $row['(orderID)'];
                    }
                    $result->free();
                }
            } while ($this->db_manager->db_link->next_result());
        } else {
    ...
    public function deleteOrder($orderID, $userID) {
        ...
        $result = $this->db_manager->executeScalar('SELECT COUNT(*) as `count` FROM ' . Constants::TBL_ORDERS . ' where ' . Constants::ORDERS_ID . ' = ' . $orderID . ' AND ' . Constants::ORDERS_BUYER_ID . ' = ' . $userID . ' LIMIT 1');
        $count = $result;
        if ($count > 0) {
            if (!$mainResut = $this->db_manager->queryWithResult('SELECT * FROM ' . Constants::TBL_ORDERITEMS . ' where ' . Constants::ORDERITEMS_ORDERID . ' = ' . $orderID)) {
            ...
            while ($row = $mainResut->fetch_assoc()) {
                ...
                if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCTS . "` SET `" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "` = " . Constants::PRODUCTS_FLD_AVA_QUANTITY . " + $quantity, `" . Constants::PRODUCTS_FLD_EARNINGS . "` = " . Constants::PRODUCTS_FLD_EARNINGS . " - $totalproductprice WHERE`" . Constants::PRODUCTS_FLD_ID . "` = '$productId' LIMIT 1"))) {
                    return $this->returnError(Constants::ORDERS_DELETE_FAILED, "Please try again later!", 0, 0, 0);
                }
            }

            if (-1 == $this->db_manager->queryWithNoResult('UPDATE ' . Constants::TBL_ORDERS . ' SET ' . Constants::ORDERS_STATUS_ID . ' = ' . Constants::ORDER_DELETED . ' WHERE ' . Constants::ORDERS_ID . ' = ' . $orderID)) {
                return $this->returnError(Constants::ORDERS_DELETE_FAILED, "Please try again later!", 0, 0, 0);
            }

            if (-1 == ($this->db_manager->queryWithNoResult('DELETE FROM ' . Constants::TBL_DELIVERYREQUESTS . ' WHERE ' . Constants::DELIVERYREQUESTS_ORDERID . ' = ' . $orderID))) {
                return $this->returnError(Constants::ORDERS_DELETE_FAILED, "Please try again later!", 0, 0, 0);
            }
    ...
    public function updateOrder($id, $status) {
        ...
        $result = $this->db_manager->executeScalar('SELECT COUNT(*) as `count` FROM ' . Constants::TBL_ORDERS . ' where ' . Constants::ORDERS_ID . ' = ' . $id . ' LIMIT 1');
        ...
        //Update the Order
        //$this->db_manager->queryWithResult('UPDATE ' . Constants::TBL_ORDERS . ' set ' . Constants::ORDERS_BUYER_ID . ' = ' . $buyerId . ' , ' . Constants::ORDERS_COST . ' = ' . $cost . ' , ' . Constants::ORDERS_DATE . " = '" . $dueDate . "' , " . Constants::ORDERS_STATUS_ID . ' = ' . $status . ' where ' . Constants::ORDERS_ID . ' = ' . $id);
        $this->db_manager->queryWithNoResult('UPDATE ' . Constants::TBL_ORDERS . ' set ' . Constants::ORDERS_STATUS_ID . ' = ' . $status . ' where ' . Constants::ORDERS_ID . ' = ' . $id);

        if ($status == Constants::ORDER_DELIVERED) {

            //Insert avg rate into DB
            $mainResult = $this->db_manager->queryWithResult('SELECT ' . Constants::ORDERITEMS_PRODUCTID . ' FROM ' . Constants::TBL_ORDERITEMS . ' WHERE ' . Constants::ORDERITEMS_ORDERID . ' = ' . $id);

            while ($row = $mainResult->fetch_assoc()) {
                $productID = $row[Constants::ORDERITEMS_PRODUCTID];
                $result = $this->db_manager->queryWithResult('SELECT AVG(' . Constants::RATE_FLD_RATE . ') as `rate`, ' . Constants::ORDERS_BUYER_ID . ' FROM ' . Constants::TBL_RATE . ', ' . Constants::TBL_ORDERS . ' WHERE ' . Constants::RATE_FLD_PRODUCT_ID . ' = ' . $productID . ' AND ' . Constants::RATE_FLD_USER_ID . ' != ' . Constants::ORDERS_BUYER_ID . ' AND ' . Constants::TBL_ORDERS . "." . Constants::ORDERS_ID . ' = ' . $id);
                if (!$result)
                    return $this->returnError(Constants::ORDERS_UPDATE_FAILED, "Please try again later!");
                $row = $result->fetch_assoc();
                ...
                if (!$avgRate || $avgRate == null)
                    $avgRate = 0;
                $deletePreviousResult = $this->db_manager->queryWithNoResult('DELETE FROM ' . Constants::TBL_RATE . ' WHERE ' . Constants::RATE_FLD_PRODUCT_ID . " = '$productID' AND  " . Constants::RATE_FLD_USER_ID . " = '$buyer_id'");
                $result = $this->db_manager->queryWithNoResult("INSERT INTO " . Constants::TBL_RATE . " SET " . Constants::RATE_FLD_PRODUCT_ID . " = '$productID', " . Constants::RATE_FLD_USER_ID . " = '$buyer_id', " . Constants::RATE_FLD_RATE . " = '$avgRate'");
                if (-1 == $result)
                    return $this->returnError(Constants::ORDERS_UPDATE_FAILED, "Please try again later!");
            }
            $result = $this->db_manager->queryWithNoResult('DELETE FROM ' . Constants::TBL_DELIVERYREQUESTS . ' WHERE ' . Constants::DELIVERYREQUESTS_ORDERID . " = '$id' LIMIT 1");
            if (-1 == $result)
                return $this->returnError(Constants::ORDERS_UPDATE_FAILED, "Please try again later!");
        }
    ...
    public function getOrderItems($orderID, $buyerID) {
        ...
        $result = $this->db_manager->executeScalar('SELECT COUNT(*) as `count` FROM ' . Constants::TBL_ORDERS . ' where ' . Constants::ORDERS_ID . ' = ' . $orderID . ' and ' . Constants::ORDERS_BUYER_ID . ' = ' . $buyerID . ' LIMIT 1');
        if ($result > 0) {
            ...
            $mainResult = $this->db_manager->queryWithResult($query);
    ...
    public function addCategory($name) {
        ...
        if (strlen($name) != 0) {
            ...
            if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_NAME . "` = '$name' LIMIT 1"))) {
            ...
            if ($result == 1) {
                ...
                if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_CATEGORIES . "` SET `" . Constants::CATEGORIES_FLD_NAME . "` = '$name' "))) {
                    return $this->returnError(Constants::CATEGORY_INSERT_FAILED, "Please try again later!");
                } else {
                    $theResponse = new Response(Constants::CATEGORY_ADD_SUCCESS, $this->db_manager->getLastInsertID(), "");
                    return(json_encode($theResponse));
                }
            }
        } else {
        ...
        if (strlen($id) != 0) {
            ...
            if ($check->fetch_assoc()['count'] == 0) {
                if (-1 == ($result = $this->db_manager->queryWithNoResult("DELETE FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_ID . "` = '$id' LIMIT 1"))) {
                    ...
                    // successul response
                    $theResponse = new Response(Constants::CATEGORY_DELETE_SUCCESS, "", "");
                    return(json_encode($theResponse));
                }
            } else {
    ...
    public function selectCategory($id) {
        if (strlen(trim($id)) != 0) {
            $id = Utilities::makeInputSafe($id);
            if (!$result = $this->db_manager->queryWithResult("SELECT * FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_ID . "` = '$id' LIMIT 1")) {
    ...
    public function updateCategory($id, $name) {
        ...
        if ((strlen($id) != 0) && (strlen($name) != 0)) {
            ...
            if (!($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_ID . "`=" . $id))) {
            ...
            if ($result == 1) {
                ...
                if ($result == 1) {
                    ...
                    $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_CATEGORIES . "` SET `" . Constants::CATEGORIES_FLD_NAME . "` = '$name' " . "WHERE `" . Constants::TBL_CATEGORIES . "`.`" . Constants::CATEGORIES_FLD_ID . "` = $id");
    ...
    public function getAllCategories() {
        if (!$result = $this->db_manager->queryWithResult('SELECT * FROM `' . Constants::TBL_CATEGORIES . '`')) {
    ...
    public function addCategorySpec($categoryId, $name) {
        ...
        if ((strlen(trim($name)) != 0) && (strlen(trim($categoryId)) != 0)) {
            ...
            if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_ID . "` = '$categoryId' LIMIT 1"))) {
                return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
            }
            //check if name is repeated
            if ($result == 1) {
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_NAME . "` = '$name' AND `" . Constants::CATEGORIES_SPEC_FLD_CATID . "` = '$categoryId' LIMIT 1"))) {
                    return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
                }
                ...
                if ($result == 1) {
                    ...
                    if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_CATEGORIES_SPEC . "` (`" . Constants::CATEGORIES_SPEC_FLD_CATID . "` ,`" . Constants::CATEGORIES_SPEC_FLD_NAME . "`) VALUES ( '$categoryId','$name' )"))) {
                        return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
                    } else {
                        $theResponse = new Response(Constants::CATEGORY_SPECS_ADD_SUCCESS, $this->db_manager->getLastInsertID(), "");
                        return(json_encode($theResponse));
                    }
                }
            } else {
    ...
    public function updateCategorySpec($id, $categoryId, $name) {
        if ((strlen(trim($id)) != 0) && (strlen(trim($categoryId)) != 0) && (strlen(trim($name)) != 0)) {
            ...
            $categoryId = Utilities::makeInputSafe($categoryId);
            //check if id exists
            if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_ID . "` = '$id' LIMIT 1"))) {
                return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
            }
            if ($result == 1) {
                ...
                if ($result == 1) {
                    ...
                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_CATID . "` = '$categoryId' AND " . Constants::CATEGORIES_SPEC_FLD_NAME . " = '$name' AND `" . Constants::CATEGORIES_SPEC_FLD_ID . "` != $id LIMIT 1"))) {
                    ...
                    if ($result == 1) {
                        ...
                        $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_CATEGORIES_SPEC . "` SET `" . Constants::CATEGORIES_SPEC_FLD_CATID . "` = '$categoryId' , `" . Constants::CATEGORIES_SPEC_FLD_NAME . "`= '$name'  WHERE " . Constants::CATEGORIES_SPEC_FLD_ID . "= '$id' ");
    ...
    public function deleteCategorySpec($id) {
        if (strlen(trim($id)) != 0) {
            ...
            if (-1 == ($result = $this->db_manager->queryWithNoResult("DELETE FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_ID . "` = '$id' LIMIT 1"))) {
    ...
    public function getCategorySpec($id) {
        $id = Utilities::makeInputSafe($id);
        if (strlen($id) != 0) {
            if (!$result = $this->db_manager->queryWithResult("SELECT * FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_CATID . "` = '$id'")) {
                return $this->returnError(Constants::CATEGORY_SELECT_FAILED, "Please try again later!");
            }
            $ret = array();
            while ($row = $result->fetch_assoc()) {
    ...
    public function updateRate($buyerId, $productId, $rate) {
        ...
        if ((strlen($rate) != 0) && (strlen($productId) != 0) && (strlen($rate) != 0)) {
            ...
            if (($rate > 5) || ($rate <= 0)) {
                ...
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_BUYERS . "` WHERE `" . Constants::BUYERS_FLD_USER_ID . "` = '$buyerId' LIMIT 1"))) {
                    return $this->returnError(Constants::RATE_INSERT_FAILED, "Please try again later!");
                }

                if ($result == 1) {

                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_PRODUCTS . "` WHERE `" . Constants::PRODUCTS_FLD_ID . "` = '$productId' LIMIT 1"))) {
                        return $this->returnError(Constants::RATE_INSERT_FAILED, "Please try again later!");
                    }
                    if ($result == 1) {
                        // Select rate if exists if not insert it
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO " . Constants::TBL_RATE . " SET " . Constants::RATE_FLD_PRODUCT_ID . " = '$productId', " . Constants::RATE_FLD_USER_ID . " = '$buyerId', " . Constants::RATE_FLD_RATE . " = '$rate' ON DUPLICATE KEY UPDATE " . Constants::RATE_FLD_RATE . " = $rate"))) {
                            return $this->returnError(Constants::RATE_INSERT_FAILED, "Please try again later!");
                        }
                        ...
                        if (!$result = $this->db_manager->queryWithNoResult($qy)) {
                            return $this->returnError(Constants::RATE_INSERT_FAILED, "Please try again later!");
                        }
    ...
    public function getAllProducts($cateID = -1, $sellerID = -1, $isAdmin = false) {
        ...
        if ($isAdmin) {
            $query = "SELECT p.* FROM `products_view` p ORDER BY p." . Constants::PRODUCTS_FLD_ID . " DESC";
        } else if ($sellerID > 0) {
            ...
            $query = "SELECT p.* FROM `products_view` p WHERE p." . Constants::PRODUCTS_FLD_SELLER_ID . " = '$sellerID' AND p." . Constants::PRODUCTS_FLD_AVAILABILITY_ID . " != " . Constants::PRODUCT_DELETED . " ORDER BY p." . Constants::PRODUCTS_FLD_ID . " DESC";
        } else {
            if ($cateID > 0) {
                ...
                $query = "SELECT p.* FROM `products_view` p INNER JOIN " . Constants::TBL_USERS . " u ON u." . Constants::USERS_FLD_ID . " = p." . Constants::PRODUCTS_FLD_SELLER_ID . " AND u." . Constants::USERS_FLD_STATUS . " = 1 WHERE p." . Constants::PRODUCTS_FLD_AVAILABILITY_ID . " != " . Constants::PRODUCT_DELETED . " AND p." . Constants::PRODUCTS_FLD_CATEGORY_ID . " = '$cateID' ORDER BY p." . Constants::PRODUCTS_FLD_ID . " DESC";
            }
        }
        if (!$result = $this->db_manager->queryWithResult($query)) {
    ...
    public function addProductSpec($productId, $specs) {
        ...
        if ($result == 1) {
            foreach ($specs as $spec) {
                ...
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES_SPEC . "` WHERE `" . Constants::CATEGORIES_SPEC_FLD_ID . "` = '$specid' AND  `" . Constants::CATEGORIES_SPEC_FLD_NAME . "` = '$specname' LIMIT 1"))) {
                    return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
                }
                if ($result == 1) {
                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_PRODUCT_SPEC . "` WHERE `" . Constants::PRODUCT_SPEC_FLD_PRODUCT_ID . "` = '$productId' AND  `" . Constants::PRODUCT_SPEC_FLD_CAT_ID . "` = '$specid' LIMIT 1"))) {
                        return $this->returnError(Constants::CATEGORY_SPECS_INSERT_FAILED, "Please try again later!");
                    }
                    if ($result == 0) {
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_PRODUCT_SPEC . "` ( `" . Constants::PRODUCT_SPEC_FLD_PRODUCT_ID . "`,`" . Constants::PRODUCT_SPEC_FLD_CAT_ID . "`,`" . Constants::PRODUCT_SPEC_FLD_VALUE . "`) VALUES ('$productId' , '$specid' ,'$specvalue' )"))) {
    ...
    public function updateProductSpec($id, $newSpecValue) {
        if ((strlen(trim($id)) != 0) && (strlen(trim($newSpecValue)) != 0)) {
            ...
            //check if id exists 
            if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_PRODUCT_SPEC . "` WHERE `" . Constants::PRODUCT_SPEC_FLD_ID . "` = '$id' LIMIT 1"))) {
                return $this->returnError(Constants::PRODUCT_SPECS_UPDATE_FAILED, "Please try again later!");
            }
            if ($result == 1) {
                $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCT_SPEC . "` SET `" . Constants::PRODUCT_SPEC_FLD_VALUE . "` = '$newSpecValue'  WHERE `" . Constants::PRODUCT_SPEC_FLD_ID . "` = '$id' ");
    ...
    public function getProductSpec($productId) {
        if (strlen(trim($productId)) != 0) {
            $productId = Utilities::makeInputSafe($productId);
            if (!$result = $this->db_manager->queryWithResult('SELECT * FROM `' . Constants::TBL_PRODUCT_SPEC . "` WHERE `" . Constants::PRODUCT_SPEC_FLD_PRODUCT_ID . "` = '$productId' ")) {
    ...
    public function addProduct($name, $price, $size, $weight, $available_quantity, $origin, $provider, $image, $seller_id, $category_id, $specs, $description) {
        ...
        if ((strlen(trim($name)) != 0) && (strlen(trim($price)) != 0) && (strlen(trim($size)) != 0) && (strlen(trim($weight)) != 0) && (strlen(trim($available_quantity)) != 0) &&
            ...
            if ((is_numeric($price) && ($price >= 0)) && (is_numeric($weight) && ($weight >= 0)) && (is_numeric($available_quantity)) && ($available_quantity >= 0)) {
                ...
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_SELLERS . "` WHERE `" . Constants::SELLERS_FLD_USER_ID . "` = '$seller_id' LIMIT 1"))) {
                    return $this->returnError(Constants::PRODUCT_ADD_FAILED, "Please try again late1r!");
                }
                if ($result == 1) {
                    //check if cat id (foreign key) exists
                    if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_CATEGORIES . "` WHERE `" . Constants::CATEGORIES_FLD_ID . "` = '$category_id' LIMIT 1"))) {
                        return $this->returnError(Constants::PRODUCT_ADD_FAILED, "Please try again later!");
                    }
                    if ($result == 1) {
                        //inserting              
                        if (-1 == ($result = $this->db_manager->queryWithNoResult("INSERT INTO `" . Constants::TBL_PRODUCTS . "` ( `" . Constants::PRODUCTS_FLD_NAME . "`,`" . Constants::PRODUCTS_FLD_PRICE . "`,`" . Constants::PRODUCTS_FLD_SIZE . "`,`" . Constants::PRODUCTS_FLD_WEIGHT . "`,`" . Constants::PRODUCTS_FLD_AVA_QUANTITY . "`,`" . Constants::PRODUCTS_FLD_ORIGIN . "`,`" . Constants::PRODUCTS_FLD_PROVIDER . "`,`" . Constants::PRODUCTS_FLD_IMAGE . "`,`" . Constants::PRODUCTS_FLD_SELLER_ID . "`,`" . Constants::PRODUCTS_FLD_CATEGORY_ID . "`,`" . Constants::PRODUCTS_FLD_DESCRIPTION . "`) VALUES ('$name' , '$price', '$size' , '$weight' , '$available_quantity' , '$origin' , '$provider' , '$image' , '$seller_id' , '$category_id', '$description' )"))) {
                            return $this->returnError(Constants::PRODUCT_ADD_FAILED, "Please try again later!");
                        } else {
                            $productID = $this->db_manager->getLastInsertID();
                            ...
                            if ($addSpecResult['statusCode'] == Constants::PRODUCT_SPEC_ADD_SUCCESS) {
                                $theResponse = new Response(Constants::PRODUCT_ADD_SUCCESS, $this->db_manager->getLastInsertID(), "");
                                return(json_encode($theResponse));
                            } else {
                                if (-1 == ($result = $this->db_manager->queryWithNoResult("DELETE FROM `" . Constants::TBL_PRODUCTS . "` WHERE `" . Constants::PRODUCTS_FLD_ID . "` = '$productID' LIMIT 1"))) {
                                    return $this->returnError(Constants::PRODUCT_ADD_FAILED, "Please try again later!");
                                }
                                return $this->returnError(Constants::PRODUCT_ADD_FAILED, "Please try again later!");
                            }
    ...
    public function deleteProduct($productId, $isAdmin = false, $sellerID = -1) {
        ...
        if (strlen(trim($productId)) != 0) {
            ...
            if (!$isAdmin && $sellerID > 0) {
                ...
                if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE `" . Constants::TBL_PRODUCTS . "` SET `" . Constants::PRODUCTS_FLD_AVAILABILITY_ID . "` = " . Constants::PRODUCT_DELETED . "  WHERE `" . Constants::PRODUCTS_FLD_ID . "` = '$productId' AND `" . Constants::PRODUCTS_FLD_SELLER_ID . "` = '$sellerID'"))) {
                ...
                if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE " . Constants::TBL_ORDERS . " o JOIN " . Constants::TBL_ORDERITEMS . " ot ON ot." . Constants::ORDERITEMS_ORDERID . " = o." . Constants::ORDERS_ID . " JOIN " . Constants::TBL_PRODUCTS . " p ON ot." . Constants::ORDERITEMS_PRODUCTID . " = p." . Constants::PRODUCTS_FLD_ID . " SET o." . Constants::ORDERS_COST . " = o." . Constants::ORDERS_COST . " - (p." . Constants::PRODUCTS_FLD_PRICE . " * ot." . Constants::ORDERITEMS_QUANTITY . ") WHERE p." . Constants::PRODUCTS_FLD_ID . " = '$productId'"))) {
                    return $this->returnError(Constants::PRODUCT_DELETE_FAILED, "Please try again later!");
                }
    ...
    public function updateProduct($id, $name, $price, $size, $weight, $availability_id, $available_quantity, $origin, $provider, $image, $seller_id, $desc, $more) {
        ...
        if ((strlen(trim($id)) != 0) && (strlen(trim($name)) != 0) && (strlen(trim($price)) != 0) && (strlen(trim($availability_id)) != 0) && (strlen(trim($size)) != 0) && (strlen(trim($weight)) != 0) && (strlen(trim($available_quantity)) != 0) && (strlen(trim($origin)) != 0) && (strlen(trim($provider)) != 0) && (strlen(trim($image)) != 0) && (strlen(trim($seller_id)) != 0) && (strlen(trim($desc)) != 0)) {
            ...
            if ((is_numeric($price) && ($price >= 0)) && (is_numeric($weight) && ($weight >= 0)) && (is_numeric($available_quantity)) && ($available_quantity >= 0)) {
                ...
                //check if id exists 
                if (-1 == ($result = $this->db_manager->executeScalar("SELECT COUNT(*) as `count` FROM `" . Constants::TBL_PRODUCTS . "` WHERE `" . Constants::PRODUCTS_FLD_ID . "` = '$id' LIMIT 1"))) {
                ...
                if ($result == 1) {
                    ...
                    if (!$result = $this->db_manager->executeScalar("SELECT  COUNT(*) as `count` FROM `" . Constants::TBL_AVAILABILITY_STATUS . "` WHERE `" . Constants::AVAILABILITY_FLD_ID . "` = '$availability_id' LIMIT 1")) {
                        return $this->returnError(Constants::PRODUCT_UPDATE_FAILED, "Please try again later!");
                    }
                    if ($result == 1) {
                        //check if seller's  id (foreign key) exists
                        if (!$result = $this->db_manager->executeScalar("SELECT  COUNT(*) as `count` FROM `" . Constants::TBL_SELLERS . "` WHERE `" . Constants::SELLERS_FLD_USER_ID . "` = '$seller_id' LIMIT 1")) {
                            return $this->returnError(Constants::PRODUCT_UPDATE_FAILED, "Please try again later!");
                        }
                        if ($result == 1) {

                            //check if cat id (foreign key) exists
                            if (-1 == ($result = $this->db_manager->queryWithNoResult("UPDATE " . Constants::TBL_PRODUCTS . " SET " . Constants::PRODUCTS_FLD_NAME . " = '$name' ," . Constants::PRODUCTS_FLD_PRICE . " = '$price' ," . Constants::PRODUCTS_FLD_SIZE . " = '$size' ," . Constants::PRODUCTS_FLD_WEIGHT . " = '$weight' ," . Constants::PRODUCTS_FLD_AVAILABILITY_ID . " = '$availability_id' ," . Constants::PRODUCTS_FLD_AVA_QUANTITY . " = '$available_quantity' ," . Constants::PRODUCTS_FLD_ORIGIN . " = '$origin' ," . Constants::PRODUCTS_FLD_PROVIDER . " = '$provider' ," . Constants::PRODUCTS_FLD_IMAGE . " = '$image', " . Constants::PRODUCTS_FLD_DESCRIPTION . " = '$desc' WHERE `" . Constants::PRODUCTS_FLD_SELLER_ID . "` = '$seller_id' AND " . Constants::PRODUCTS_FLD_ID . " = '$id'"))) {
                                return $this->returnError(Constants::PRODUCT_UPDATE_FAILED, "Please try again later!");
                            } else {
                    ...
                    return $this->returnError(Constants::PRODUCT_INVALID_PRODUCT_ID, "Please try again later!");
                }
            } else {
                return $this->returnError(Constants::PRODUCT_INVALID_PARAMETER, "Please ,enter a valid numeric value");
            }
        } else {
            return $this->returnError(Constants::PRODUCT_UPDATE_EMPTY_DATA, "Please,all fields are required.");
        }
    }
    ...
    public function getProductByKey($keyword) {
        if (strlen(trim($keyword)) != 0) {
            ...
            $query = "SELECT p.* FROM `products_view` p WHERE p." . Constants::PRODUCTS_FLD_NAME . " LIKE '%$keyword%' OR p." . Constants::PRODUCTS_FLD_DESCRIPTION . " LIKE '%$keyword%' ORDER BY " . Constants::PRODUCTS_FLD_SOLDITEMS . " DESC, p." . Constants::PRODUCTS_FLD_ID;
            if (!$result = $this->db_manager->queryWithResult($query)) {
    ...
    public function getTop3In4Cat() {
        ...
        if (!$result = $this->db_manager->queryWithResult('SELECT ' . Constants::PRODUCTS_FLD_CATEGORY_ID . " from " . Constants::TBL_PRODUCTS . " group by " . Constants::PRODUCTS_FLD_CATEGORY_ID . " order by sum(" . Constants::PRODUCTS_FLD_SOLDITEMS . ") DESC limit 4")) {
            ...
            foreach ($categoriesIdArray as $cateID) {
                ...
                $query = "SELECT p.* FROM `products_view` p INNER JOIN " . Constants::TBL_USERS . " u ON u." . Constants::USERS_FLD_ID . " = p." . Constants::PRODUCTS_FLD_SELLER_ID . " AND u." . Constants::USERS_FLD_STATUS . " = 1 WHERE p." . Constants::PRODUCTS_FLD_CATEGORY_ID . " = '$cateID'  ORDER BY " . Constants::PRODUCTS_FLD_SOLDITEMS . " DESC , p." . Constants::PRODUCTS_FLD_ID;
